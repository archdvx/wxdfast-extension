...
var wxdfastIntegratorNotFound = true;
var disposition = '';
var hostName;
var wxdfastIntegratorVersion;
var wxdfastVersion = '';
var chromeVersion;
var firefoxVersion;
...
function readStorage() {
    current_browser.storage.sync.get(function (items) {
        ...
        if (items["wxdfast-urls-exclude"]) {
            urlsToSkip = items["wxdfast-urls-exclude"].split(/[\s,]+/);
        } else {
            current_browser.storage.sync.set({ "wxdfast-urls-exclude": '' });
        }

        // Read the storage for included keywords
        if (items["wxdfast-urls-include"]) {
            urlsToInterrupt = items["wxdfast-urls-include"].split(/[\s,]+/);
        } else {
            current_browser.storage.sync.set({ "wxdfast-urls-include": '' });
        }

        if (items["wxdfast-mime-exclude"]) {
            mimeToSkip = items["wxdfast-mime-exclude"].split(/[\s,]+/);
        } else {
            current_browser.storage.sync.set({ "wxdfast-mime-exclude": '' });
        }

        // Read the storage for included keywords
        if (items["wxdfast-mime-include"]) {
            mimeToInterrupt = items["wxdfast-mime-include"].split(/[\s,]+/);
        } else {
            current_browser.storage.sync.set({ "wxdfast-mime-include": '' });
        }

        // Read the storage for the minimum file-size to interrupt
        if (items["wxdfast-min-file-size"]) {
            minFileSizeToInterrupt = parseInt(items["wxdfast-min-file-size"]);
        } else {
            current_browser.storage.sync.set({ "wxdfast-min-file-size": minFileSizeToInterrupt });
        }

        // Read the storage for enabled flag
        if (!items["wxdfast-interrupt"]) {
            // Keep the value string
            current_browser.storage.sync.set({ "wxdfast-interrupt": 'true' });
        } else {
            var interrupt = (items["wxdfast-interrupt"] == "true");
...
function createContextMenus() {
    current_browser.contextMenus.create({
        title: current_browser.i18n.getMessage("downloadWith"),
        id: "download_with_wxdfast",
        contexts: ['link']
    });

    current_browser.contextMenus.create({
        title: current_browser.i18n.getMessage("downloadAll"),
        id: "download_all_links_with_wxdfast",
        contexts: ['page']
    });
    ...
    current_browser.contextMenus.onClicked.addListener(function (info, tab) {
        ...
        } else if (info.menuItemId === "download_media_with_wdfast") {
...
function setDownloadHooks() {
    ...
    current_browser.downloads.onCreated.addListener(function (downloadItem) {
        ...
        if (wxdfastIntegratorNotFound || !interruptDownloads) { // wxdfast-integrator not installed
            return;
        }
    ...
    current_browser.webRequest.onHeadersReceived.addListener(function (details) {
        ...
        if (wxdfastIntegratorNotFound) { // wxdfast-integrator not installed
            return {
...
////////////////// Utility Functions //////////////////
/**
 * Send message to wxdfast-integrator
 */
function sendMessageToHost(message) {
    current_browser.runtime.sendNativeMessage(hostName, message, function (response) {
        clearMessage();
        wxdfastIntegratorNotFound = (response == null);
        if (!wxdfastIntegratorNotFound && !wxdfastIntegratorVersion) {
            wxdfastIntegratorVersion = response.Version;
            wxdfastVersion = response.wxdfast;
        }
        changeIcon();
    });
}
...
function getState() {
    if (wxdfastIntegratorNotFound || !wxdfastIntegratorVersion) {
        return 2;
    } else if (!wxdfastIntegratorVersion.startsWith(REQUIRED_INTEGRATOR_VERSION)) {
        return 1;
    } else {
        return 0;
    }
...
function updateExcludeKeywords(exclude) {
    ...
    current_browser.storage.sync.set({ "wxdfast-urls-exclude": exclude });
}
...
function updateIncludeKeywords(include) {
    ...
    current_browser.storage.sync.set({ "wxdfast-urls-include": include });
}
...
function updateExcludeMIMEs(exclude) {
    ...
    current_browser.storage.sync.set({ "wxdfast-mime-exclude": exclude });
}
...
function setInterruptDownload(interrupt, writeToStorage) {
    ...
    if (writeToStorage) {
        current_browser.storage.sync.set({ "wxdfast-interrupt": interrupt.toString() });
    }
    changeIcon();
}
